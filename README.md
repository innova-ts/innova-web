...
2. Colors
    * The colors are defined in `assets\css\main.css`, under the style rules for `@theme`:
    ...
    ```
    > 🚩 **RECOMENDATION**: I suggest you do not add or remove any color from the current definition, just replace the current values with the new ones as you need. 
    
    > 🚩 **RECOMENDATION**: Do not replace the current values with random colors, the current colors are calculated for a reason. Instead, you can extract your branding colors from your logo using some specific tools like [Adobe Color](https://color.adobe.com/es/create/image), or some AI platform such as [ChatGPT](https://chatgpt.com/), [Gemini](https://gemini.google.com/app), etc. 
